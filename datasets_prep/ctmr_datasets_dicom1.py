...
class CBCTDataset_sobel(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        npimg = (npimg - dispaly0) / (dispaly1 - dispaly0)
        nplabs = (nplabs - nplabs.min()) / (nplabs.max() - nplabs.min())
        npimg = npimg.astype(np.float32)
        nplabs = nplabs.astype(np.float32)
...
class CBCTDataset_canny(data.Dataset):
    def __init__(self, path, size = None):
        self.img = os.listdir(path)  # [:4]
        self.size = size
        self.path = path
    ...
    def __getitem__(self, item):
        ...
        if fft == True:
            ...
            nplabs = cv2.bilateralFilter(nplabs,gau_random,75,75)
            #print("blur:"+str(gau_random))
        ...
        canny_random = random.randint(50,180)
        top_random = random.randint(10,200-canny_random)
        #print("canny:"+str(canny_random))

        nplabs = np.uint8(nplabs)
        nplabs =  cv2.Canny(nplabs, canny_random, canny_random+top_random)
...
class CBCTDataset_test(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        #npimg99 = np.percentile(npimg, 96)
        #npimg[npimg > npimg99] = npimg99
